...
| 1.5 | ~~Long pages mix many concerns, making them hard to test and maintain.~~ | Done | `SellerDashboardPage` refactored from ~896 lines to 217 lines; tab components extracted to `src/components/seller-dashboard/`. `AdminPage` refactored from 498 lines to 65 lines; sections extracted to `src/components/admin/`. `CreateListingPage` (722 lines) and `MessagesPage` (657 lines) remain. |
