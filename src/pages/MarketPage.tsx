...
function SpeciesCard({ item }: { item: ReturnType<typeof getMarketOverview>['trending_up'][0] }) {
  return (
    ...
    >
      ...
      <p className="text-xs text-zinc-500 mb-3">{item.sales_count} sales</p>
      <Sparkline data={item.sparkline_data} width={200} height={40} />
      <div className="flex items-center justify-between mt-2">
        <span className="text-sm">{Math.round(item.current_median).toLocaleString()} THB</span>
  ...
  return (
    <div className="pt-24 pb-16 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-10">
          <h1 className="text-3xl sm:text-4xl font-light tracking-tight mb-2">Market Overview</h1>
          <p className="text-zinc-500">Real-time price data for Thailand's plant market</p>
        </div>
        ...
        <div className="bg-zinc-900/30 border border-white/5 rounded-xl p-6 mb-10">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
            <div>
              ...
              <p className="text-sm text-zinc-500">Price History & Volume</p>
            </div>
            <select
        ...
        <div className="space-y-10">
          <section>
            <SectionHeader icon={TrendingUp} title="Trending Up" color="text-emerald-400" />
          ...
          <section>
            <SectionHeader icon={TrendingDown} title="Trending Down" color="text-red-400" />
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          ...
          <section>
            <SectionHeader icon={Activity} title="Most Traded" color="text-blue-400" />
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          ...
          <section>
            <SectionHeader icon={Flame} title="Hot Right Now" color="text-orange-400" />
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          ...
          <section>
            <SectionHeader icon={Snowflake} title="Cooling Off" color="text-cyan-400" />
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
          ...
          {/* High Value Sales */}
          <section>
            <SectionHeader icon={DollarSign} title="High-Value Sales" color="text-amber-400" />
            <div className="bg-zinc-900/30 border border-white/5 rounded-xl overflow-hidden">
              ...
              {market.high_value_sales.map(tx => (
                <div key={tx.id} className="grid grid-cols-4 gap-4 px-6 py-3 text-sm border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                  ...
                  <span className="text-right font-medium">{tx.sale_price_thb.toLocaleString()} THB</span>
