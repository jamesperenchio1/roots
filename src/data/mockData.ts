...
export const TRANSACTIONS: Transaction[] = [
  ...
  { id: 't-13', listing_id: 'l-1', buyer_id: 'u-5', seller_id: 'u-1', plant_id: 'p-1', sale_price_thb: 5500, platform_fee_thb: 440, seller_payout_thb: 5060, status: 'pending_verification', delivery_method: 'ship', created_at: '2024-06-19', payment_slip_url: '/images/payment/slip1.jpg', payment_ref_number: 'PP20240619ABC123' },
];
...
// Merge scraped price data if available
function mergeScrapedSnapshots(): void {
  try {
    // Dynamic import to avoid breaking if files don't exist
    const scrapedModule = import.meta.glob<{ default: Array<{ species_id: string; snapshot_date: string; median_price_thb: number; mean_price_thb: number; min_price_thb: number; max_price_thb: number; sale_count: number; source: string }> }>('./scraped_prices.json', { eager: true });
    const scraped = Object.values(scrapedModule)[0]?.default || [];
    for (const s of scraped) {
      PRICE_SNAPSHOTS.push({
        id: `scraped-${s.species_id}-${s.snapshot_date}`,
        species_id: s.species_id,
        size_category: undefined,
        snapshot_date: s.snapshot_date,
        median_price_thb: s.median_price_thb,
        mean_price_thb: s.mean_price_thb,
        min_price_thb: s.min_price_thb,
        max_price_thb: s.max_price_thb,
        sale_count: s.sale_count,
        created_at: s.snapshot_date,
      });
    }
  } catch {
    // scraped_prices.json may not exist yet
  }
}
mergeScrapedSnapshots();
...
// ---------------------------------------------------------------------------
export interface ExternalListingView {
  id: string;
  title: string;
  price_thb: number;
  image_url?: string;
...
export function getExternalListings(): ExternalListingView[] {
  try {
    const mod = import.meta.glob<{ default: ExternalListingView[] }>('./scraped_listings.json', { eager: true });
    const data = Object.values(mod)[0]?.default || [];
    return data.slice(0, 50); // cap at 50 for performance
  } catch {
    return [];
  }
}
