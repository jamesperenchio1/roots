...
export default function DisputePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      setTimeout(() => router.push(`/order/${transactionId}`), 2000);
    } catch (err) {
    ...
    return (
      <div className="pt-24 pb-16 px-4 text-center">
        <div className="max-w-md mx-auto">
          ...
          <Link href={`/order/${transactionId}`} className="text-emerald-400 hover:underline text-sm">{t('checkout:dispute.backToOrder')}</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pt-24 pb-16 px-4 sm:px-6">
      <div className="max-w-lg mx-auto">
        <Link href={`/order/${transactionId}`} className="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-white mb-6 transition-colors">
