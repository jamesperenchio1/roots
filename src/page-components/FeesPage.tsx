...
import { PLATFORM_FEE_PERCENT, calculatePlatformFees } from '@/lib/platform-config';
...
export default function FeesPage() {
  ...
  const { platformFeeThb: exampleFee, sellerPayoutThb: exampleReceive } = calculatePlatformFees(EXAMPLE_PRICE);
  const sellerPriceLabel = PLATFORM_FEE_PERCENT === 0 ? t('common:fees.sellers.free') : `${PLATFORM_FEE_PERCENT}%`;
    ...
    <div className="pt-24 pb-16 px-4 sm:px-6">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-zinc-500">{t('common:fees.subtitle', { percent: PLATFORM_FEE_PERCENT })}</p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6 mb-12">
          ...
          <div className="bg-zinc-900/30 border border-white/5 rounded-xl p-8">
            ...
            <p className="text-4xl font-light text-amber-400 mb-1">{sellerPriceLabel}</p>
            <p className="text-sm text-zinc-500 mb-4">{t('common:fees.sellers.note', { percent: PLATFORM_FEE_PERCENT })}</p>
            <ul className="space-y-3">
          ...
          <div className="bg-zinc-900/30 border border-white/5 rounded-xl p-6 mb-8">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span>{t('common:fees.example.soldForValue', { value: EXAMPLE_PRICE.toLocaleString(), currency: t('common:currency') })}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-zinc-400">{t('common:fees.example.platformFee', { percent: PLATFORM_FEE_PERCENT })}</span>
                <span className="text-red-400">{t('common:fees.example.platformFeeValue', { value: `-${exampleFee.toLocaleString()}`, currency: t('common:currency') })}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-zinc-400">{t('common:fees.example.youReceive')}</span>
                <span className="text-emerald-400 font-medium">{t('common:fees.example.youReceiveValue', { value: exampleReceive.toLocaleString(), currency: t('common:currency') })}</span>
              </div>
            </div>
          </div>
        )}
