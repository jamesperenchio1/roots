        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full py-20">
          <div className="max-w-2xl">
            ...
            <div className={`flex flex-wrap gap-4 transition-all duration-1000 delay-700 ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              ...
              <Link href="/seller-dashboard/listings/new" className="inline-flex items-center gap-2 border border-white/20 text-white px-6 py-3 rounded-full text-sm font-medium hover:bg-white/5 transition-all">
      ...
      <section className="pt-20 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/browse?all=1" className="hidden sm:flex items-center gap-1 text-sm text-emerald-400 hover:text-emerald-300 transition-colors">
          ...
          {listings.length === 0 ? (
            <div className="text-center py-16 bg-zinc-900/30 border border-white/5 rounded-xl">
              ...
              <Link href="/seller-dashboard/listings/new" className="text-emerald-400 text-sm hover:underline">{t('common:actions.addNew')}</Link>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      <section className="py-20 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/market" className="hidden sm:flex items-center gap-1 text-sm text-emerald-400 hover:text-emerald-300 transition-colors">
              {t('common:actions.seeAll')} <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
              ...
              {(market?.trending_up.length ?? 0) === 0 ? (
                ...
                <div className="space-y-3">
                  {market?.trending_up.slice(0, 5).map((item, i) => (
                    <Link href={`/species/${item.species.id}`} key={i} className="flex items-center justify-between p-3 bg-black/30 rounded-lg hover:bg-black/50 transition-colors">
                      <div>
      ...
      <section className="py-20 px-4 sm:px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-8 sm:p-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            ...
            <Link href="/seller-dashboard/listings/new" className="inline-flex items-center gap-2 bg-emerald-500 text-black px-6 py-3 rounded-full text-sm font-medium hover:bg-emerald-600 transition-colors">
